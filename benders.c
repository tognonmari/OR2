...
void ben_patching(const multitour_sol* curr_sol, multitour_sol* patched_sol, const instance* inst) {
	...
	char figure_name[64];
	generate_name(figure_name, sizeof(figure_name), "figures/ben_%d_%d_%d_prepatch.png", inst->nnodes, inst->randomseed, (int) get_timer());
	...
	generate_name(figure_name, sizeof(figure_name), "figures/ben_%d_%d_%d_postpatch.png", inst->nnodes, inst->randomseed, (int)get_timer());
	plot_multitour(inst->verbose >= 200, (const multitour_sol*)patched_sol, inst->nnodes, inst->nodes, figure_name); //plot di debug
	//
	free(start);
}
